...
def test_derive_throw_aerial_fields(con):
    throws = throw_in._throws_taken(con, 100, "C1")
    assert [t["has_aerial"] for t in throws] == [False, False, False, True, True]
    assert [t["aerial_won"] for t in throws] == [False, False, False, True, False]
...
def test_derive_throw_distance_and_territory(con):
    ...
    assert distances == [11.1803, 5.0, 5.0, 5.0, 2.0]
    assert territories == [10, 5, -5, 5, 2]
...
def test_retention_rate(con):
    ...
    # Retained: throws 1 and 3 out of 5 -> 40%.
...
def test_distance_metric(con):
    ...
    assert result.value == pytest.approx((11.1803399 + 5 + 5 + 5 + 2) / 5)
    assert result.sample_size == 5
...
def test_retention_allowed_defensive_mirror(con):
    ...
    assert result.value == pytest.approx(40.0)  # same 2/5 as retention_rate
    assert result.sample_size == 5
